...
const cols = ['Draft Legal', 'Ironclad', 'Harvey', 'Spellbook'] as const

const rows: { label: string; values: ('yes' | 'no' | 'partial' | string)[] }[] = [
  { label: 'Open source', values: ['yes', 'no', 'no', 'no'] },
  { label: 'Self-host', values: ['yes', 'no', 'no', 'no'] },
  { label: 'Full lifecycle (intake → obligations)', values: ['yes', 'yes', 'partial', 'no'] },
  { label: 'AI agents (not just LLM features)', values: ['yes', 'partial', 'yes', 'partial'] },
  { label: 'Transparent pricing', values: ['yes', 'no', 'no', 'partial'] },
  { label: 'Time to first contract', values: ['minutes', '6+ months', 'weeks', 'days'] },
]
                ...
                {cols.map((c) => (
                  <th
                    ...
                    className={cn(
                      ...
                      c === 'Draft Legal' && 'bg-emerald-50 text-emerald-800'
                    )}
                  >
                    {c}
                  </th>
